...
export default function Register() {
  ...
  const { register, user, loginAsGuest } = useAuth();
  ...
  return (
    <div className="min-h-screen bg-surface-base flex">
      ...
      <div className="hidden lg:flex w-1/2 bg-surface-raised items-center justify-center p-12 relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-purple-600/5 via-transparent to-accent/5" />
        <div className="absolute top-1/4 -right-20 w-80 h-80 bg-purple-600/5 rounded-full blur-[120px]" />
        <div className="absolute bottom-1/4 -left-20 w-80 h-80 bg-accent/5 rounded-full blur-[120px]" />
        <div className="relative z-10 max-w-md text-center">
          ...
          <motion.h1 initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-4xl font-bold text-white mb-4">Begin Your Journey</motion.h1>
          <motion.p initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-text-secondary text-lg">Ready to Dock Your Career at the Right Opportunity?</motion.p>
        </div>
      </div>
      <div className="flex-1 flex items-center justify-center p-8">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm">
          <div className="card-premium-lg p-8">
            ...
            <p className="text-text-muted text-sm mb-8">Start your job discovery journey</p>
            ...
            <p className="text-center text-sm text-text-muted mt-6">Already have an account? <Link to="/login" className="text-accent-light hover:text-accent font-medium transition-colors">Sign In</Link></p>
          </div>
        </motion.div>
      </div>
